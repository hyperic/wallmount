dojo.provide("hyperic.tests.sandbox.RangeSlider");
...
(function(){
    ...
    dojo.declare("hyperic.form._RangeSliderMixin", null, {
        ...
        setLowColor: function(color){
            dojo.style(this.progressBarBefore, "background", color);
        },
        setMiddleColor: function(color){
            dojo.style(this.progressBar, "background", color);
        },
        setHighColor: function(color){
            dojo.style(this.progressBarAfter, "background", color);
        },
        ...
        postCreate: function(){
            ...
            var _self = this;
            var mover = dojo.declare(hyperic.form._SliderMoverMax, {
            ...
            // a dnd for the bar!
            var barMover = dojo.declare(hyperic.form._SliderBarMover, {
                constructor: function(){
                    this.widget = _self;
                }
            });
        ...
        _printSliderBar: function(priorityChange, isMaxVal){
            ...
            if (priorityChange && this.slideDuration > 0 && this.progressBar.style[this._progressPixelSize]){
                ...
                propsBarAfter[this._handleOffsetCoord] = { start: this.progressBarAfter.style[this._handleOffsetCoord], end: progressBarVal + ((percentMax - percentMin) * 100), units:"%"};
                propsBarAfter[this._progressPixelSize] = { start: this.progressBarAfter.style[this._progressPixelSize], end: ((1 - percentMax) * 100), units:"%"};

                propsBarBefore[this._handleOffsetCoord] = { start: this.progressBarBefore.style[this._handleOffsetCoord], end: 0, units:"%"};
                propsBarBefore[this._progressPixelSize] = { start: this.progressBarBefore.style[this._progressPixelSize], end: progressBarVal, units:"%"};
                ...
                var animBarAfter = dojo.animateProperty({node: this.progressBarAfter,duration: duration, properties: propsBarAfter});
                var animBarBefore = dojo.animateProperty({node: this.progressBarBefore,duration: duration, properties: propsBarBefore});
                var animCombine = dojo.fx.combine([animHandle, animHandleMax, animBar, animBarAfter, animBarBefore]);
                animCombine.play();
            }else{
                ...
                this.progressBarBefore.style[this._handleOffsetCoord] = "0%";
                this.progressBarBefore.style[this._progressPixelSize] = progressBarVal + "%";

                this.progressBarAfter.style[this._handleOffsetCoord] = progressBarVal + ((percentMax - percentMin) * 100) + "%";
                this.progressBarAfter.style[this._progressPixelSize] = ((1 - percentMax) * 100) + "%";
            }
        }
    });

    dojo.declare("hyperic.form._SliderMoverMax", dijit.form._SliderMover, {
    ...
    dojo.declare("hyperic.form._SliderBarMover", dojo.dnd.Mover, {
    ...
    dojo.declare("hyperic.form.HorizontalRangeSlider",
        [dijit.form.HorizontalSlider, hyperic.form._RangeSliderMixin],
        {
            // summary:
            //  A form widget that allows one to select a range with two horizontally draggable images
            templateString: dojo.cache('hyperic.tests.sandbox','resources/HorizontalRangeSlider.html')
        }
    );
